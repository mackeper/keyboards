...
module.exports = {
  ...
  body: (p) => {
    ...
    const drill = `
      (pad "1" thru_hole circle locked (at 0 0) (size ${p.outline} ${p.outline}) (drill ${p.drill}) (layers *.Cu *.Mask))
    `;
    ...
    for (let i = 0; i < 8; i++) {
      ...
      vias += `
        (pad "1" thru_hole circle locked (at ${x} ${y}) (size ${p.vias_drill * 1.5} ${p.vias_drill * 1.5}) (drill ${p.vias_drill}) (layers *.Cu *.Mask))
      `;
    }
